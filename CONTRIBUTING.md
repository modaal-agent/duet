...
7. **The published version is the tag — for both flavors, by construction.** Swift: SwiftPM resolves the git tag itself. Kotlin: the tag push triggers `.github/workflows/publish.yml`, which builds with `-PpublishVersion=<tag>` and lands the release on the family's static Maven host (`https://modaal-agent.github.io/maven`) — the version literal in `kotlin/build.gradle.kts` is the *mavenLocal development default only* and nothing published reads it. Cutting `X.Y.Z` therefore means: write the changelog entry, move the dev-default literal to `X.Y.Z-SNAPSHOT` (it names the current line for anyone iterating against mavenLocal), commit, tag that commit, push the tag. A published version is immutable: the publish workflow refuses to overwrite an existing `<artifact>/<version>/` path — a bad release is followed by a new version, never a re-tag.

   The failure the tag-derived scheme exists to prevent: the `0.1.1` cut left `version = "0.1.0-SNAPSHOT"`, so tags `0.1.0` and `0.1.1` published the same mutable coordinate. Nothing in either flavor's build complained — the tags differ in git, and to Maven they are one artifact that the last `publishToMavenLocal` overwrites. An adopter pinned to `0.1.1` built against the `0.1.0` jar and failed at `Unresolved reference` on symbols `0.1.1` had graduated, with no signal pointing at the version. A hand-moved literal can lag the tag; a version passed from the tag cannot.
