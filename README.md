...
> **Status: pre-release.** Both flavors and the toolchain have landed (`swift/`, `kotlin/`, the [`duet-tools`](https://github.com/modaal-agent/duet-tools) CLI) together with the normative contracts (`contracts/`). The Swift packages resolve by URL at a tag; the Kotlin artifacts resolve from the family's static Maven repository (`https://modaal-agent.github.io/maven` — see `kotlin/README.md` for the repository block). Treat the API surface as a preview: pre-1.0 minors are breaking by family convention, so pin exactly.
